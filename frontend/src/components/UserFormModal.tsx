    ...
    const mutation = useMutation({
        mutationFn: (validatedData: ApiPayload) => {
            if (mode === 'add') {
                ...
                // Explicitly delete the password property.
                delete payload.password;
                // This is now correctly typed as Omit<ApiPayload, 'password'>
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-lg w-full">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label className="block text-sm">Role</label><select className="w-full border rounded p-2 text-gray-900" {...register('role')}><option value="user">User</option><option value="supervisor">Supervisor</option><option value="admin">Admin</option><option value="superadmin">Super Admin</option></select>{errors.role && <p className="text-xs text-red-500">{errors.role.message}</p>}</div>
                    ...
                    {mode === 'add' && <p className="text-sm text-blue-600 p-3 bg-blue-50 rounded-md">A "Welcome & Set Password" email will be sent to the user upon creation.</p>}
